...
const FacultyLogin = ({ setShowFacultyLogin }) => {
  ...
  const API_URL =
    "https://excel-result-1.onrender.com" || "http://localhost:5000";
  ...
  const fetchStudentsList = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const filteredStudents = data.results.filter(
          (student) =>
  ...
  const handleStudentSelect = async (studentRegNo) => {
    ...
    try {
      const response = await fetch(
        `${API_URL}/api/results/student/${studentRegNo}`
      );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setError("Failed to fetch student details. Please check the register number.");
      }
    } catch (err) {
      console.error("API Error:", err);
      setError("Network error. Please try again.");
    ...
    return (
      <div className="max-w-6xl mx-auto p-6 text-gray-800">
        ...
        <div className="bg-white rounded-lg shadow-lg p-6 mb-6 text-gray-800">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
            <div>
              ...
              <p>
                <strong>Registration No:</strong> {selectedStudent.studentRegNo}
              </p>
              <p>
                <strong>Section:</strong>{" "}
                {selectedStudent.studentSection || "N/A"}
              </p>
              <p>
                <strong>Email:</strong>{" "}
                {selectedStudent.studentEmail || "Not provided"}
              </p>
            </div>
            <div>
              <p>
                <strong>Year:</strong> {selectedStudent.studentYear || "N/A"}
              </p>
              <p>
                <strong>Date of Birth:</strong>{" "}
                {selectedStudent.studentDOB || "N/A"}
              </p>
              <p>
                <strong>Overall CGPA:</strong>{" "}
                <span className="text-green-600 font-bold text-lg">
                  {selectedStudent.overallCGPA}
                </span>
              </p>
              <p>
        ...
        {selectedStudent.notificationHistory &&
          selectedStudent.notificationHistory.length > 0 && (
                ...
                {selectedStudent.notificationHistory
                  ...
                  .map((notification, index) => (
                    ...
                    >
                      <div>
                        <span>
                          Semester {notification.semesterDetected} result
                          notified
                        </span>
                        <div className="text-sm text-gray-500">
                          Telegram:{" "}
                          {notification.notificationStatus.telegram
                            ? "✅"
                            : "❌"}{" "}
                          | Email:{" "}
                          {notification.notificationStatus.email ? "✅" : "❌"}
                        </div>
                      </div>
  ...
  if (
    ...
    return (
      <div className="max-w-4xl mx-auto p-6 text-gray-800">
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-2xl font-bold text-gray-800">
            Year {selectedYear} - Section {selectedSection} Students (
            {studentsList.length})
          </h2>
          <button
            onClick={resetToClassSelection}
      ...
      <form action="">
        <div>
          <h1 className="text-black ">
            Enter a Paraticular Student Register Number
          </h1>
          <div className="flex gap-4 my-4 justify-around">
            <input
              type="text"
              className="text-black border-2 border-black w-1/2 py-2 text-center "
              placeholder="Student Register Number"
              value={studentRegNo}
              onChange={(e) => setStudentRegNo(e.target.value)}
            />
            <button  className="text-blue-600 hover:text-blue-800 px-3 py-1 rounded bg--blue50 cursor-pointer" onClick={handleSubmit}>Submit</button>
          </div>
        </div>
      </form>
        ...
        <div className="space-y-4">
          ...
          {selectedYear && selectedSection && (
            ...
            >
              {loading
                ? "Loading Students..."
                : `View Year ${selectedYear} Section ${selectedSection} Students`}
            </button>
          )}
          {/* 
